...
class ViewController: UIViewController {
    ...
    @IBAction private func thirdExampleButtonTapped() {
        ...
        do {
            try client.fetch(url: imageURL).border(parameters).toJson() { [weak self] (result) in
                switch (result) {
                case .success(let data):
                    DispatchQueue.main.async {
                        let dictionary = try? JSONSerialization.jsonObject(with: data, options: .mutableContainers)
    ...
    @IBAction private func fourthExampleButtonTapped() {
        ...
        do {
            try client.fetch(url: imageURL).border(parameters).toImageData() { [weak self] (result) in
                switch (result) {
                case .success(let data):
                    DispatchQueue.main.async {
                        let image = UIImage(data: data)
